...
/**
 * Route Handler for getting forecast data.
 *
 * GET /api/forecast
 *
 * Requires session and organization.
 * Returns the forecast data for the organization.
 */
export async function GET(request: NextRequest) {
  ...
  const forecastData = await withTenant(ctx, async (tx) => {
    ...
    // Map to the full Opportunity contract shape the rule modules accept.
    ...
    const { buildForecast, totalPipeline, totalWeightedForecast } =
      await import('@quotapilot/domain/rules/forecast');
    ...
    const quarterOpportunities = domainOpportunities.filter((op) => {
      ...
      return (
        closeDate >= currentQuarterStart && closeDate <= currentQuarterEnd && op.stage !== 'lost'
      );
    });

    const quarterCommitted = quarterOpportunities.reduce((sum, op) => sum + op.amount, 0);
    const quarterWeighted = quarterOpportunities.reduce(
      (sum, op) =>
        sum +
        op.amount *
